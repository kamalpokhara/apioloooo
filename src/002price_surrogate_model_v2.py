...
# ---- retrain with best hyperparameters, evaluated properly using validation set ----
...
print('avg_price_lag1' in feature_cols)
print('volume_lag1' in feature_cols)

print("\n Feature Columns: ", feature_cols)

print("\n BEST Hyperparameters: ", best_params)
importance = pd.Series(booster.feature_importance(importance_type='gain'), index=feature_cols)
print(importance.sort_values(ascending=False).head(15))

# testing on month 10, unseen data- final test, tested once, 
print("\n\nFINAL TEST on month 10, unseen data")

final_params = {
    'num_leaves': 63, 'learning_rate': 0.08454062300182051,
    'subsample': 0.7168028395554554, 'reg_lambda': 4.526019285774548,
    'objective': 'regression_l1', 'verbosity': -1, 'random_state': 42
}

print("\n FINAL TEST Hyperparameters: ", final_params)

train_set = lgb.Dataset(X_train, label=y_train, categorical_feature=cat_cols, free_raw_data=False)
valid_set = lgb.Dataset(X_valid, label=y_valid, reference=train_set, categorical_feature=cat_cols, free_raw_data=False)

booster = lgb.train(final_params, train_set, num_boost_round=1000,
                     valid_sets=[valid_set], callbacks=[lgb.early_stopping(50)])

pred_test_price = np.expm1(booster.predict(X_test, num_iteration=booster.best_iteration))
actual_test_price = test[target_col].values

mae_test = mean_absolute_error(actual_test_price, pred_test_price)
rmse_test = mean_squared_error(actual_test_price, pred_test_price) ** 0.5
r2_test = r2_score(actual_test_price, pred_test_price)
mape_test = np.mean(np.abs((actual_test_price - pred_test_price) / actual_test_price)) * 100
print(f"TEST — MAE: {mae_test:.2f}  RMSE: {rmse_test:.2f}  MAPE: {mape_test:.2f}%  R2: {r2_test:.3f}")

persist_test = test['avg_price_lag1'].fillna(train['avg_price'].mean())
mae_persist_test = mean_absolute_error(actual_test_price, persist_test)
prod_mean = train.groupby('product_name')['avg_price'].mean()
mean_pred_test = test['product_name'].map(prod_mean).fillna(train['avg_price'].mean())
...
# # 2. build full-data model and save it as the production model
# # train validate test model
# full_train = pd.concat([train, valid, test]).copy()
# for c in cat_cols:
#     full_train[c] = full_train[c].astype('category')

# X_full = full_train[feature_cols]
# y_full = np.log1p(full_train[target_col])

# full_set = lgb.Dataset(X_full, label=y_full, categorical_feature=cat_cols, free_raw_data=False)
# final_booster = lgb.train(final_params, full_set, num_boost_round=booster.best_iteration)
# final_booster.save_model('../models/price_surrogate_final.txt')
